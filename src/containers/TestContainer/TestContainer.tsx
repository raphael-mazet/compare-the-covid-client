...
export default function TestContainer(): JSX.Element {
  ...
  //NOTE: useQuery hook example - run immediately when called. Pulls from server
  const { data: userData, loading, error: userDataError } = useQuery<QueryData>(
    GET_USER_BY_ID,
    {
      variables: {
        id: 3,
  ...
  const [
    saveUser,
    { data: savedUserData, error: savedUserError },
    ...
    <>
      <div>
        {loading ? (
          ...
          <p>Hello {userData?.getUserbyId.username}</p>
        )}
      </div>
      <form onSubmit={handleSubmit}>
        <label htmlFor="username">username</label>
      ...
      <div>
        {savedUserData && (
          <p>
            You have created {savedUserData?.createUser.username}, id:
            {savedUserData?.createUser.id}
          </p>
        )}
        {savedUserError && <p>Error: {savedUserError.message}</p>}
      </div>
